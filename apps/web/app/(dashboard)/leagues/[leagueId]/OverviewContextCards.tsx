...
export function OverviewContextCards({ members, upcomingMatches, leagueSlug, leagueMongoId, base }: Props) {
  ...
  return (
    <>
      ...
      {/* Mini standings */}
      <div className="rounded-xl overflow-hidden" style={{ border: '1px solid rgb(var(--c-border-subtle))' }}>
        <div className="flex items-center justify-between px-4 py-3" style={{ borderBottom: '1px solid rgb(var(--c-border-subtle))' }}>
          <p className="text-[11px] font-semibold uppercase tracking-widest" style={{ color: 'rgb(var(--c-text-3))' }}>Standings</p>
        ...
        {members.map((m, i) => (
          <ContextCard
            ...
            style={{ borderBottom: i < members.length - 1 ? '1px solid rgb(var(--c-overlay-sm))' : undefined }}
          >
            <div
            ...
            >
              <span className="text-[11px] font-bold w-5 text-center shrink-0"
                style={{ color: i === 0 ? '#f5c842' : i === 1 ? '#a0a0a0' : i === 2 ? '#c87533' : 'rgb(var(--c-text-3))' }}>
                {i < 3 ? ['🥇','🥈','🥉'][i] : `${i + 1}`}
              </span>
              ...
              <p className="flex-1 text-[12px] font-medium truncate" style={{ color: m.isMe ? 'rgb(var(--c-text-1))' : 'rgb(var(--c-text-2))' }}>
                {m.name}
                {m.isMe && <span style={{ color: 'rgb(217 119 87)', fontSize: '10px' }}> you</span>}
              </p>
              <p className="text-[12px] font-semibold font-mono shrink-0" style={{ color: 'rgb(var(--c-text-1))' }}>{m.totalPoints}</p>
            </div>
          </ContextCard>
        ))}
      </div>
      ...
      {upcomingMatches.length > 0 && (
        <div className="rounded-xl overflow-hidden" style={{ border: '1px solid rgb(var(--c-border-subtle))' }}>
          <div className="flex items-center justify-between px-4 py-3" style={{ borderBottom: '1px solid rgb(var(--c-border-subtle))' }}>
            <p className="text-[11px] font-semibold uppercase tracking-widest" style={{ color: 'rgb(var(--c-text-3))' }}>Coming Up</p>
          ...
          {upcomingMatches.map((match, i) => {
            ...
            return (
              <ContextCard
                ...
                style={{ borderBottom: i < upcomingMatches.length - 1 ? '1px solid rgb(var(--c-overlay-sm))' : undefined }}
              >
                <div className="px-4 py-3 flex items-center gap-3 transition-colors hover:bg-white/[0.02]">
                  <div className="flex-1 min-w-0">
                    <p className="text-[12px] font-medium truncate" style={{ color: 'rgb(var(--c-text-1))' }}>
                  ...
                  {match.status === 'finished' && (
                    <span className="text-[10px] px-2 py-0.5 rounded-full"
                      style={{ background: 'rgb(var(--c-overlay-md))', color: 'rgb(var(--c-text-3))' }}>FT</span>
                  )}
                </div>
              </ContextCard>
            )

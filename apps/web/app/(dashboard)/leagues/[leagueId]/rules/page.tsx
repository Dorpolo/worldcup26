...
export default async function RulesPage({ params }: Props) {
  ...
  return (
    <div className="h-full overflow-y-auto p-5">
      <div className="max-w-2xl mx-auto rules-prose">
        <ReactMarkdown
          remarkPlugins={[remarkGfm]}
          components={{
            h1: ({ children }) => (
              <h1 className="text-xl font-bold mb-4 mt-2" style={{ color: 'rgb(240 235 227)' }}>{children}</h1>
            ),
            h2: ({ children }) => (
              <h2 className="text-[14px] font-semibold mt-6 mb-2" style={{ color: 'rgb(240 235 227)' }}>{children}</h2>
            ),
            h3: ({ children }) => (
              <h3 className="text-[13px] font-semibold mt-4 mb-1" style={{ color: 'rgb(200 196 190)' }}>{children}</h3>
            ),
            p: ({ children }) => (
              <p className="text-[13px] leading-relaxed mb-2" style={{ color: 'rgb(160 152 144)' }}>{children}</p>
            ),
            blockquote: ({ children }) => (
              <blockquote
                className="pl-4 py-1 my-2 text-[12px] italic"
                style={{ borderLeft: '2px solid rgb(217 119 87 / 0.5)', color: 'rgb(160 152 144)' }}
              >
                {children}
              </blockquote>
            ),
            ...
            li: ({ children }) => (
              <li className="flex gap-2.5 text-[13px] py-0.5">
                ...
                <span style={{ color: 'rgb(160 152 144)' }}>{children}</span>
              </li>
            ),
            hr: () => (
              <hr className="my-4" style={{ borderColor: 'rgb(255 255 255 / 0.08)' }} />
            ),
            strong: ({ children }) => (
              <strong style={{ color: 'rgb(240 235 227)', fontWeight: 600 }}>{children}</strong>
            ),
            code: ({ children }) => (
              <code
            ...
            thead: ({ children }) => (
              <thead style={{ borderBottom: '1px solid rgb(255 255 255 / 0.1)' }}>{children}</thead>
            ),
            th: ({ children }) => (
              <th
            ...
            td: ({ children }) => (
              <td
                className="py-2 px-3"
                style={{ color: 'rgb(160 152 144)', borderBottom: '1px solid rgb(255 255 255 / 0.04)' }}
              >
                {children}
              </td>
            ),
